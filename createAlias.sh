...
then
	...
	lineServerAlias=`grep -n "ServerAlias" /etc/apache2/sites-available/${domain}.conf | tail -1 | awk -F":" '{print $1}'`
	echo -e "\e[1;33mWill insert ServerAlias on line $((lineServerAlias+1))\e[0m"
else
	echo -e "\e[1;32mNo ServerAlias present\e[0m"
	lineServerAlias=`grep -n "ServerName ${domain}" /etc/apache2/sites-available/${domain}.conf | awk -F":" '{print $1}'`
	echo -e "\e[1;33mWill insert ServerAlias on line $((lineServerAlias+1))\e[0m"
fi
...
if [[ -z "$3" ]];
then
	sed -i "$((lineServerAlias+1))iServerAlias $2" /etc/apache2/sites-available/${domain}.conf
	...
	sed -i "$((lineServerAlias+1))iServerAlias $2" /etc/apache2/sites-available/${domain}.conf
	sed -i "s/ServerAlias $2/\tServerAlias $2/g" /etc/apache2/sites-available/${domain}.conf
	sed -i "$((lineServerAlias+2))iServerAlias www.$2" /etc/apache2/sites-available/${domain}.conf
	sed -i "s/ServerAlias www.$2/\tServerAlias www.$2/g" /etc/apache2/sites-available/${domain}.conf
fi
